...
class SatelliteDataLoader(ShapeNetDataLoader):
    """Reuses ShapeNet's per-sample (rendering_images, volume) format.

    Folder layout expected:
      <root>/<sample_name>/rendering/NN.png
      <root>/<sample_name>/model.binvox
      <root>/Satellite.json
    """
    def __init__(self, cfg):
        self.dataset_taxonomy = None
        # Path templates take only sample_name (taxonomy is ignored).
        self.rendering_image_path_template = cfg.DATASETS.SATELLITE.RENDERING_PATH
        self.volume_path_template = cfg.DATASETS.SATELLITE.VOXEL_PATH
        with open(cfg.DATASETS.SATELLITE.TAXONOMY_FILE_PATH, encoding='utf-8') as fh:
            self.dataset_taxonomy = json.loads(fh.read())
    ...
    def get_files_of_taxonomy(self, taxonomy_folder_name, samples):
        ...
        for sample_name in samples:
            ...
            if not os.path.exists(volume_file_path):
                print('[WARN] %s Skip %s: voxel %s missing' % (dt.now(), sample_name, volume_file_path))
                continue
            img_dir = os.path.dirname(self.rendering_image_path_template % (sample_name, 0))
            if not os.path.isdir(img_dir):
                print('[WARN] %s Skip %s: image dir %s missing' % (dt.now(), sample_name, img_dir))
                continue
            n_views = len([n for n in os.listdir(img_dir) if n.lower().endswith('.png')])
            rendering_paths = [self.rendering_image_path_template % (sample_name, i) for i in range(n_views)]
            rendering_paths = [p for p in rendering_paths if os.path.exists(p)]
            if not rendering_paths:
                continue
            out.append({
                'taxonomy_name': taxonomy_folder_name,
                'sample_name': sample_name,
                'rendering_images': rendering_paths,
                'volume': volume_file_path,
            })
        return out
