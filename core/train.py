...
def train_net(cfg):
    ...
    output_dir = os.path.join(cfg.DIR.OUT_PATH, '%s', dt.now().strftime('%Y-%m-%dT%H-%M-%S'))  # Windows-safe (no colons)
    ...
    for epoch_idx in range(init_epoch, cfg.TRAIN.NUM_EPOCHES):
        ...
        # Validate the training models (every VAL_FREQ epochs, or on the last)
        val_freq = getattr(cfg.TRAIN, "VAL_FREQ", 1)
        if (epoch_idx + 1) % val_freq == 0 or (epoch_idx + 1) == cfg.TRAIN.NUM_EPOCHES:
            iou = test_net(cfg, epoch_idx + 1, output_dir, val_data_loader, val_writer, encoder, decoder, refiner, merger)
        else:
            iou = best_iou  # skip val this epoch
